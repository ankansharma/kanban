...
export function Column({ column, onRename, onAdd, onDelete }: ColumnProps) {
  ...
  return (
    <section
      ...
      className={`flex min-h-0 min-w-[260px] flex-1 flex-col rounded-xl border border-slate-200 bg-white/80 shadow-sm ${
      ...
      <header className="border-b border-slate-100 px-4 pb-3 pt-4">
        ...
        {editing ? (
          <input
            ...
            className="w-full rounded-md border border-blue px-2 py-1 text-base font-semibold text-navy focus:outline-none focus:ring-2 focus:ring-blue/30"
          />
        ) : (
          <button
            type="button"
            ...
            className="w-full rounded-md text-left text-base font-semibold text-navy hover:text-blue focus:outline-none focus:ring-2 focus:ring-blue/30"
          >
            {column.title}
          </button>
        )}
